...
- 2026-09-05 `0.1.8` 已上传，包大小 `233.3KB`；针对 `0.1.6` 审核反馈增加小程序内隐私指引入口和持久化明确同意，未同意时训练会话、后续提交及使用埋点均不发送。全量测试 `305/305` 通过；微信后台隐私指引、审核专用试用码和录屏仍需人工填写后重新提审。
...
3. `0.1.8` 已上传，但仍需在微信后台补全隐私指引、提供审核专用试用码和录屏并重新提审；在此之前不宣称该修订已经对普通用户生效。
