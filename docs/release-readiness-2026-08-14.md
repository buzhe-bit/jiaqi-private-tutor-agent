...
- 2026-08-24 当前分支代码已通过微信官方 CLI 上传为 `0.1.1`，AppID 为 `wxfa3953c780a246d8`，包大小 `218.6KB`，上传成功。
- 待完成：微信后台将开发版本 `0.1.1` 设为体验版、添加体验成员并用真机扫码确认。浏览器自动化受已保存的 `mp.weixin.qq.com` 访问限制，无法代操作；需管理员在微信后台操作。
...
3. `0.1.1` 已上传，但体验版后台设置、体验成员添加和真机扫码仍未完成；浏览器自动化受已保存的 `mp.weixin.qq.com` 访问限制，需管理员在微信后台操作。在此之前不宣称小程序已经完成线上发布验收。
